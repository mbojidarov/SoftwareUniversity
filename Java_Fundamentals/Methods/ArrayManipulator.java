...
public class ArrayManipulator {
    ...
    private static void findFirstCountOddNumbers(int[] array, int count) {
        ...
        for (int i = 0; i < array.length; i++) {
            if (array[i] % 2 != 0){
                result = expandAndAddToArray(result, array[i]); //викаме нов метод, да прави масив който
                if (result.length == count){    //е с .lenght +1 по голям и да добави числото
                    break;
                }
            }
        }
    ...
    private static void findMinOdd(int[] array) {
        ...
        int index = -1;         //за принтиране на индекса, ако е -1 значи нямаме нечетно
        for (int i = 0; i < array.length; i++) {
    ...
    private static void findMinEven(int[] array) {
        int min = Integer.MAX_VALUE;
        int index = -1;         //за принтиране на индекса, ако е -1 значи нямаме четно
        for (int i = 0; i < array.length; i++) {
            if (array[i] % 2 == 0 && min >= array[i]){
                min = array[i];
                index = i;
    ...
    private static void exchange(int[] array, int index) {
        int[] temp = temp = copyArray(array);    // подавам метод да копира масива
        int countIndex = 0;
        for (int i = index + 1; i < temp.length; i++) { //презаписвам в масива
            array[countIndex] = temp[i];                // от индекса+1 до дължината
            countIndex++;                               // на позиция countIndex
        }
        for (int i = 0; i <= index; i++) {      // останалата част презаписваме
            array[countIndex] = temp[i];        //на позиция count до позиция индекс
            countIndex++;
        }
    }

    private static int[] copyArray(int[] arr) {  //копиране масив и връщане горе
